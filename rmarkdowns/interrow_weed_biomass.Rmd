...
interrow_weed_biomass_clean <-clean_combined |>              
  ...
  mutate(interrow_weed_biomass_grams_meter = (interrow_weed_biomass * 2)) |> 
...
interrow_weed_biomass_clean |> 
  ...
  ggplot(aes(x = weed_control, y = interrow_weed_biomass_grams_meter, fill = weed_control)) +
  stat_summary(geom = "bar", fun = "mean", width = 0.7) +
  stat_summary(geom = "errorbar", fun.data = "mean_se", width = 0.2) +
  stat_summary(geom="text", fun = "MeanPlusSe", aes(label= trimws(.group)),size=6.5,vjust=-0.5) +
  labs(
    x = "Interrow weed control",
    y = expression("Interrow weed biomass" ~ (g ~ m^{-2})),
...
```{r message=FALSE}
interrow_weed_biomass_clean |> 
  left_join(cld_weed_control_fisher_log) |> 
  ggplot(aes(x = weed_control, y = interrow_weed_biomass_grams_meter, fill = weed_control)) +
  stat_summary(geom = "bar", fun = "mean", width = 0.7) +
  stat_summary(geom = "errorbar", fun.data = "mean_se", width = 0.2) +
  stat_summary(geom="text", fun = "MeanPlusSe", aes(label= trimws(.group)),size=6.5,vjust=-0.5) +
