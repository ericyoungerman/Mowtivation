...
ggsave(
  filename = here("figs", "analysis",
                  "fig_inrow-weed_biomass_box_by-site-year_kg_ha.png"),
  plot     = inrow_box_total,
  width    = 12,
  height   = 5.5,
  dpi      = 300
)
...
ggsave(
  filename = here("figs", "analysis",
                  "fig_interrow-weed_biomass_box_by-site-year_kg_ha.png"),
  plot     = interrow_box_total,
  width    = 12,
  height   = 5.5,
  dpi      = 300
)
...
# Write to CSV for supplementary tables / committee folder
readr::write_csv(
  emm_wbm_out,
  here("tables", "analysis", "tab_total-weed_biomass_emm_cld_kg_ha.csv")
)
...
```
```{r}
## Figure: Interrow weed biomass by weed management (2023 Field V; raw means with model CLDs)
## Units: kg ha^-1
...
# site-year label for 2023 Field V (adjust if needed)
sy_target <- "2023.field v"

# 1) Model-based emmeans on the response scale (kg/ha), from GLOBAL model
emm_interrow_sy <- emmeans(
  weed_interrow_glmm,
  ~ weed_trt | site_year,
  type = "response"
...
emm_interrow_v23_df <- as_tibble(emm_interrow_sy) |>
  filter(site_year == sy_target) |>
  mutate(
    weed_trt   = factor(weed_trt, levels = mow_levels),
    model_mean = response,        # kg/ha
    SE         = SE,              # kg/ha
...
# 2) CLDs for treatments within site-year (Fisher-style, "a" = highest)
cld_interrow_sy <- cld(
  emm_interrow_sy,
  adjust   = "none",
  Letters  = letters,
...
cld_interrow_v23 <- cld_interrow_sy |>
  filter(site_year == sy_target) |>
  select(weed_trt, .group)

# 3) Raw means for interrow weed biomass (2023 Field V), kg/ha
weed_biomass_field_v_2023 <- weed_biomass_clean |>
  filter(site_year == sy_target)

raw_interrow_v23 <- weed_biomass_field_v_2023 |>
  group_by(weed_trt) |>
  summarise(
    raw_mean = mean(interrow_weed_biomass_kg_ha, na.rm = TRUE),
    .groups  = "drop"
...
# 4) Merge raw means, model SE, and CLDs
plot_df_interrow_v23 <- emm_interrow_v23_df |>
  select(weed_trt, model_mean, ymin, ymax) |>
  left_join(raw_interrow_v23,  by = "weed_trt") |>
  left_join(cld_interrow_v23,  by = "weed_trt")

# 5) Plot: bars = raw means (kg/ha), error bars = model-based SE (kg/ha), letters = model CLDs
fig_interrow_v23 <- ggplot(plot_df_interrow_v23,
                           aes(x = weed_trt, y = raw_mean, fill = weed_trt)) +
  geom_col(width = 0.7, color = "black") +
  geom_errorbar(aes(ymin = ymin, ymax = ymax), width = 0.14) +
  geom_text(
    aes(y = ymax * 1.08, label = .group),
...
# 6) Save figure
ggsave(
  filename = here("figs", "analysis", "fig_interrow_weed_biomass_mowing_fieldV_2023_raw_kg_ha.png"),
  plot     = fig_interrow_v23,
  width    = 9,
  height   = 5.5,
  dpi      = 300
)
