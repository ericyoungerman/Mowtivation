...
``` {r}
# 1) read + clean master
soy_all <- readxl::read_excel("~/Github/Mowtivation/raw-data/All Treatments/combined_raw.xlsx") |>
  janitor::clean_names() |>
...
# 8) save figs
ggsave("fig_weed_biomass_mowing_raw_2023.png", width = 7.5, height = 5.5, dpi = 300)
...
# 8) save figs
ggsave("fig_interrow_biomass_mowing_raw_2023.png", width = 7.5, height = 5.5, dpi = 300)
```
...
ggplot(comp_df, aes(weed_trt, biomass, fill = position)) +
  geom_col(width = 0.75, color = "black") +
  scale_x_discrete(labels = c(
    "Rolled, no control"          = "Rolled,\nno control",
    "Rolled + mowing"             = "Rolled +\nmowing",
 ...
 scale_fill_manual(values = c("In-row" = "#0072B2", "Interrow" = "#009E73"), name = NULL)+
  ...
  theme(
    legend.position     = "bottom",
    plot.title.position = "plot",              # start at y-axis
...
ggsave(
  "fig-soybean-weed-biomass_partitioned_inrow-interrow_kgha.png",
