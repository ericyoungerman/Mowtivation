...
v=subset(combined_raw, (combined_raw$year==2023))
```
<br>
...
```{r}
wb.lmerv <- lmer( weed_biomass ~ treatment+ (1|block) , data = v)
...
## Anova
```{r}
 joint_tests(emmeans(wb.lmerv, ~ treatment))
```
...
```{r}
tukey_weed_control <- emmeans(wb.lmerv, list(pairwise ~ treatment), adjust = "none")
tukey_weed_control

cld_weed_control_tukey <- cld(emmeans(wb.lmerv, ~ treatment), adjust = "none", Letters = letters, sort = TRUE, reversed = TRUE)
