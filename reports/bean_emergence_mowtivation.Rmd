...
# Summarize raw means ± SE
raw_summary <- bean_emergence_clean |>
  group_by(weed_trt) |>
  summarise(
    mean_raw = mean(bean_emergence_hectare, na.rm = TRUE),
    se_raw   = sd(bean_emergence_hectare, na.rm = TRUE) / sqrt(n()),
    .groups  = "drop"
  )

# Get CLD letters from model
cld_emergence <- cld(
  emm_emergence,
  adjust   = "none",
  Letters  = letters,
...
# Merge raw summary with CLDs
plot_df_raw <- left_join(raw_summary, cld_emergence |> select(weed_trt, .group), by = "weed_trt")

# Define colors (same palette as model-based figure)
fill_cols <- viridis::viridis(length(mow_levels), option = "D", end = 0.9, begin = 0.1)

# Plot raw means ± SE with CLD letters
ggplot(plot_df_raw, aes(x = weed_trt, y = mean_raw, fill = weed_trt)) +
  geom_col(width = 0.7, color = "black") +
  geom_errorbar(aes(ymin = mean_raw - se_raw, ymax = mean_raw + se_raw), width = 0.14) +
  geom_text(aes(y = mean_raw + se_raw * 1.08, label = .group),
            vjust = 0, fontface = "bold", size = 6) +
  scale_fill_manual(values = fill_cols, guide = "none") +
  scale_x_discrete(labels = \(x) stringr::str_replace_all(x, " \\+ ", "\n+ ")) +
  scale_y_continuous(labels = scales::label_comma()) +
  labs(
    x = NULL,
    y = "Bean emergence (plants/ha)",
    title = "Raw treatment means ± SE (pooled across site-years)",
    caption = "Raw observed means ± SE; letters = Fisher-style CLD from model."
  ) +
  theme_classic(base_size = 18) +
  theme(
    axis.text.x  = element_text(lineheight = 0.95, margin = margin(t = 8)),
...
# Save figure
ggsave(
  filename = here::here("figs", "analysis", "fig_bean_emergence_mowing_raw.png"),
  width    = 7.5,
  height   = 5.5,
  dpi      = 300
)
