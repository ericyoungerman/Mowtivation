...
## 0) Directories for weed–soy relationship outputs ----------------------

fig_dir_rel <- here::here("analysis", "figs", "weed-soy-relationships")
tab_dir_rel <- here::here("analysis", "tables", "weed-soy-relationships")

dir.create(fig_dir_rel, showWarnings = FALSE, recursive = TRUE)
dir.create(tab_dir_rel, showWarnings = FALSE, recursive = TRUE)

## Helper: save ggplots consistently (adds prefix; defaults to .png) -------
save_fig_rel <- function(plot, filename, width = 7, height = 5, dpi = 300) {
  if (!grepl("\\.(png|pdf)$", filename)) filename <- paste0(filename, ".png")
  ggplot2::ggsave(
    filename = file.path(fig_dir_rel, paste0("weed_soy-", filename)),
    plot     = plot,
    width    = width,
    height   = height,
    dpi      = dpi,
...
## Helper: write tables consistently (adds prefix; defaults to .csv) -------
save_tab_rel <- function(df, filename) {
  if (!grepl("\\.csv$", filename)) filename <- paste0(filename, ".csv")
  readr::write_csv(df, file.path(tab_dir_rel, paste0("weed_soy-", filename)))
}
...
label_break_spaces <- function(x) stringr::str_replace_all(x, " ", "\n")
label_break_comma  <- function(x) stringr::str_replace_all(x, ", ", ",\n")
...
tidy_emm <- function(emm, ref_levels = NULL) {
  ...
  lcl_col <- intersect(c("lower.CL", "asymp.LCL", "lower"), names(emm_df))[1]
  ucl_col <- intersect(c("upper.CL", "asymp.UCL", "upper"), names(emm_df))[1]
...
row_spacing_m  <- 0.762      # 30-inch rows (m)

weed_soy_clean <- read_excel(
...
# Safety check: make sure all treatments were successfully recoded
stopifnot(!any(is.na(weed_soy_clean$weed_trt)))

weed_soy_clean <- weed_soy_clean |>
  # keep only rows with non-missing total weed biomass (needed for relationships)
  filter(!is.na(weed_biomass)) |>
  mutate(
    ## ---- WEEDS: total / in-row / interrow ----------------------------------
    weed_biomass_g_m2           = weed_biomass          * 2,   # g/m² (from 0.5 m²)
    ...
    ## ---- SOYBEAN: biomass --------------------------------------------------
    bean_biomass_g_m2    = bean_biomass * 2,
    bean_biomass_kg_ha   = bean_biomass * 20,
    bean_biomass_lb_ac   = bean_biomass_kg_ha * kg_ha_to_lb_ac,

    ## ---- SOYBEAN: adjusted yield ------------------------------------------
    bean_yield_adj_bu_acre =
      (((bean_yield / 454) / (16.4 / 43560)) / 60) * ((100 - 0.00001) / (100 - 13)),
    bean_yield_adj_lbs_acre =
      ((bean_yield / 454) / (16.4 / 43560)) * ((100 - 0.00001) / (100 - 13)),
    bean_yield_adj_kg_ha =
      ((bean_yield / 454) / (16.4 / 43560)) * 1.12085 * ((100 - 0.00001) / (100 - 13)),

    ## ---- SOYBEAN: population metrics --------------------------------------
    bean_population_two_meter = bean_population * 2,
    bean_population_hectare   = (bean_population / row_spacing_m) * 10000,
...
# Model testing
```{r}
# Model selection rule for weed–soy relationships (slope heterogeneity):
# - For each soybean response (Y) × weed metric (B), fit two candidate LMMs:
#     1) Additive (common slope):        Y ~ B + site_year + (1 | site_year:block)
#     2) Interaction (site-year slopes): Y ~ B * site_year + (1 | site_year:block)
#
# - Compare interaction vs additive using ML refits (anova() does this internally):
#     * LRT p < 0.01 OR ΔAIC_int-add ≤ -4  -> keep interaction (strong evidence slopes differ by site-year)
#     * LRT p > 0.20 AND |ΔAIC_int-add| < 2 -> keep additive (negligible interaction)
#     * Otherwise = gray zone:
#         keep additive for parsimony,
#         but report site-year-specific slopes (emtrends) and show site-year patterns in plots.
#
# Notes:
# - ΔAIC_int-add = AIC(interaction) - AIC(additive)
#     negative favors interaction; positive favors additive.
# - Final reported coefficients/emmeans are fit with REML; only the comparison uses ML.
...
```{r}
# ----------------------------------------------------------------------
# Total weed biomass vs adjusted soybean yield (kg ha^-1)
# ----------------------------------------------------------------------

reg_yield <- weed_soy_clean |>
  filter(
  ...
  transmute(
    site_year = droplevels(site_year),
    block     = droplevels(block),
    B         = weed_biomass_kg_ha,
    Y         = bean_yield_adj_kg_ha
  )

# Quick structure + range checks (useful for plots / asymptote placement)
str(reg_yield)
summary(reg_yield$B)
summary(reg_yield$Y)
range(reg_yield$B, na.rm = TRUE)
range(reg_yield$Y, na.rm = TRUE)

```
#### Mixed model selection (site-year effects; ML comparison, REML for inference)
```{r}
### 2) Mixed model selection (site-year effects; ML comparison, REML for inference)

options(contrasts = c("contr.sum", "contr.poly"))

# 1) Candidate models fit with REML (for estimation / inference) ----------
yield_add <- lmer(
  Y ~ B + site_year + (1 | site_year:block),
  data = reg_yield,
  REML = TRUE
)
...
# 2) LRT + ML AICs (anova refits with REML = FALSE internally) -----------
lrt_yield <- anova(yield_add, yield_int)  # ML comparison

AIC_add_yield <- lrt_yield$AIC[1]
AIC_int_yield <- lrt_yield$AIC[2]
deltaAIC_yield_int_add <- AIC_int_yield - AIC_add_yield   # <0 favors interaction

p_int_yield <- lrt_yield$`Pr(>Chisq)`[2]

# 3) Decision rule (same as global response summary) ---------------------
p_strong    <- 0.01
p_none      <- 0.20
dAIC_strong <- 4

interaction_class_yield <- dplyr::case_when(
  p_int_yield < p_strong | (-deltaAIC_yield_int_add) >= dAIC_strong ~ "interaction",
  p_int_yield > p_none   & abs(deltaAIC_yield_int_add) < 2          ~ "additive",
  TRUE                                                              ~ "gray_zone"
)

chosen_model_name_yield <- dplyr::case_when(
  interaction_class_yield == "interaction" ~ "Interaction: B * site_year",
  TRUE                                     ~ "Additive: B + site_year"
)

yield_mod_best <- if (chosen_model_name_yield == "Interaction: B * site_year") {
  yield_int
} else {
  yield_add
}

# 4) Save model-selection summary ---------------------------------------
model_sel_yield <- tibble(
  response_label    = "Soybean yield (kg ha^-1)",
  predictor         = "Total weed biomass (kg ha^-1)",
  model_additive    = "B + site_year + (1|site_year:block)",
  model_interaction = "B * site_year + (1|site_year:block)",
  AIC_additive      = AIC_add_yield,
  AIC_interaction   = AIC_int_yield,
  deltaAIC_int_add  = deltaAIC_yield_int_add,
  LRT_p_int         = p_int_yield,
  selected_model    = chosen_model_name_yield,
  interaction_class = interaction_class_yield
)

save_tab_rel(model_sel_yield, "yield_totalweed_mixedmodel_selection")

model_sel_yield |>
  mutate(
    across(c(AIC_additive, AIC_interaction, deltaAIC_int_add), ~ round(.x, 2)),
    LRT_p_int = round(LRT_p_int, 3)
  ) |>
  kable(
    caption = "Mixed-model selection for yield vs total weed biomass. LRT p-values and ΔAIC compare interaction vs additive fixed-effects structures using ML refits; models are fit with REML for inference. ΔAIC = AIC(int) − AIC(add)."
  ) |>
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"))

cat(
  "\nSelected model for yield vs total weed biomass:\n  ",
  chosen_model_name_yield,
  sprintf("\n  LRT p (interaction vs additive) = %.3f; ΔAIC (int - add) = %.2f [%s]\n",
          p_int_yield, deltaAIC_yield_int_add, interaction_class_yield)
)
...
```{r}
## Slope summary (emtrends): yield vs total weed biomass -----------------

# Determine whether slopes vary by site-year based on the selected structure
## Slope summary (emtrends): yield vs total weed biomass -----------------

has_interaction_yield <- chosen_model_name_yield == "Interaction: B * site_year"

if (!has_interaction_yield) {

  emm_slope <- emtrends(yield_mod_best, ~ 1, var = "B")
  tmp <- summary(emm_slope, infer = c(TRUE, TRUE)) |> as_tibble()

  trend_col <- "B.trend"
  stat_col  <- intersect(c("t.ratio", "z.ratio"), names(tmp))[1]
  has_df    <- "df" %in% names(tmp)
  has_p     <- "p.value" %in% names(tmp)

  slope_tbl <- tmp |>
    transmute(
      Scope              = "Pooled across site-years",
      slope_kg_per_kg    = .data[[trend_col]],
      slope_kg_per_100kg = 100 * .data[[trend_col]],
      SE                 = SE,
      df                 = if (has_df) df else NA_real_,
      lower_95CL         = lower.CL,
      upper_95CL         = upper.CL,
      stat               = if (!is.na(stat_col)) .data[[stat_col]] else NA_real_,
      p_value            = if (has_p) p.value else NA_real_
    )

} else {

  emm_slope <- emtrends(yield_mod_best, ~ site_year, var = "B")
  tmp <- summary(emm_slope, infer = c(TRUE, TRUE)) |> as_tibble()
...
save_tab_rel(slope_tbl, "yield_totalweed_slopes_emtrends")

slope_tbl |>
  mutate(across(where(is.numeric), ~ round(.x, 3))) |>
  kable(
    caption = "Slopes of adjusted soybean yield vs total weed biomass (emtrends from selected mixed model)"
  ) |>
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"))
...
```{r}
### 4) Linear vs hyperbolic (Ryan/Spitters biomass form; optional 3-parameter)
#### Linear vs rectangular hyperbolic response --------------------------

dat_yield <- reg_yield |>
  transmute(B = B, Y = Y) |>
  filter(is.finite(B), is.finite(Y), B >= 0)

# Linear (marginal; no random effects)
lin_mod <- lm(Y ~ B, data = dat_yield)

# Hyperbolic (Ryan/Spitters biomass form): Y = Y0 / (1 + iw * B)
start_vals_ryan <- list(
  Y0 = as.numeric(quantile(dat_yield$Y, 0.95, na.rm = TRUE)),
  iw = as.numeric(1 / max(dat_yield$B[dat_yield$B > 0], na.rm = TRUE))
)

hyper_mod_ryan <- tryCatch(
  minpack.lm::nlsLM(
    Y ~ Y0 / (1 + iw * B),
    data    = dat_yield,
    start   = start_vals_ryan,
    lower   = c(Y0 = 0, iw = 0),
...
# Optional: 3-parameter absolute-yield hyperbola: Y = Y0 - (Lmax*B)/(K+B)
start_vals_3p <- list(
  Y0   = as.numeric(quantile(dat_yield$Y, 0.95, na.rm = TRUE)),
  Lmax = as.numeric(max(dat_yield$Y, na.rm = TRUE) - min(dat_yield$Y, na.rm = TRUE)),
  K    = as.numeric(median(dat_yield$B[dat_yield$B > 0], na.rm = TRUE))
)
if (!is.finite(start_vals_3p$K)) start_vals_3p$K <- 1

hyper_mod_3p <- tryCatch(
  minpack.lm::nlsLM(
    Y ~ Y0 - (Lmax * B) / (K + B),
    data    = dat_yield,
    start   = start_vals_3p,
    lower   = c(Y0 = 0, Lmax = 0, K = 0),
    control = minpack.lm::nls.lm.control(maxiter = 500)
  ),
  error = function(e) e
)

# R2 helper for nonlinear fits
r2_from_fit <- function(y, yhat) {
  rss <- sum((y - yhat)^2)
  tss <- sum((y - mean(y))^2)
  1 - rss / tss
}
...
mod_comp <- tibble(
  ...
  AIC   = AIC(lin_mod),
  R2    = broom::glance(lin_mod)$r.squared
)

# Optional: parameter table (nice for your global regression summary workflow)
param_tbl <- tibble()

if (!inherits(hyper_mod_ryan, "error")) {
  mod_comp <- bind_rows(
    mod_comp,
    tibble(
      model = "Hyperbolic (Ryan/Spitters: Y0/(1+iwB))",
      df    = df.residual(hyper_mod_ryan),
      AIC   = AIC(hyper_mod_ryan),
      R2    = r2_from_fit(dat_yield$Y, fitted(hyper_mod_ryan))
    )
  )

  param_tbl <- bind_rows(
    param_tbl,
    tibble(model = "Ryan/Spitters", term = names(coef(hyper_mod_ryan)), estimate = as.numeric(coef(hyper_mod_ryan)))
  )
} else {
  cat("\nRyan/Spitters hyperbolic fit failed:\n", hyper_mod_ryan$message, "\n")
}

if (!inherits(hyper_mod_3p, "error")) {
  mod_comp <- bind_rows(
    mod_comp,
    tibble(
      model = "Hyperbolic (3-parameter: Y0 - Lmax*B/(K+B))",
      df    = df.residual(hyper_mod_3p),
      AIC   = AIC(hyper_mod_3p),
      R2    = r2_from_fit(dat_yield$Y, fitted(hyper_mod_3p))
    )
  )

  param_tbl <- bind_rows(
    param_tbl,
    tibble(model = "3-parameter", term = names(coef(hyper_mod_3p)), estimate = as.numeric(coef(hyper_mod_3p)))
  )
} else {
  cat("\n3-parameter hyperbolic fit failed:\n", hyper_mod_3p$message, "\n")
}

save_tab_rel(mod_comp, "yield_totalweed_linear_vs_hyperbolic")
if (nrow(param_tbl) > 0) save_tab_rel(param_tbl, "yield_totalweed_hyperbolic_parameters")

mod_comp |>
  mutate(across(where(is.numeric), ~ round(.x, 3))) |>
  kable(
    caption = "Marginal model comparison (no random effects): linear vs hyperbolic yield response to weed biomass"
  ) |>
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"))
...
##### Table
```{r}
# 6) OPTIONAL: Combine current yield ~ total weed biomass outputs into one workbook
# (robust: reads the CSVs you already saved)

yield_totalweed_tables <- list(
  MixedModel_Selection = readr::read_csv(
    file.path(tab_dir_rel, "weed_soy-yield_totalweed_mixedmodel_selection.csv"),
    show_col_types = FALSE
  ),
  Slopes_emtrends = readr::read_csv(
    file.path(tab_dir_rel, "weed_soy-yield_totalweed_slopes_emtrends.csv"),
    show_col_types = FALSE
  ),
  Linear_vs_Hyperbolic = readr::read_csv(
    file.path(tab_dir_rel, "weed_soy-yield_totalweed_linear_vs_hyperbolic.csv"),
    show_col_types = FALSE
  )
)

writexl::write_xlsx(
  yield_totalweed_tables,
  path = file.path(tab_dir_rel, "weed_soy-yield_totalweed_all-tables.xlsx")
)
...
##### Figure
```{r}
#### Visualisation (clean: linear + 2 hyperbolas + 3p asymptote text)

p_yield_totalweed <- ggplot(dat_yield, aes(x = B, y = Y)) +
  geom_point(color = "black") +
  # Linear fit (quiet benchmark)
  stat_function(
  ...
  labs(
    ...
    y = bquote("Soybean yield (kg ha"^-1*")"),
    title = "Yield vs total weed biomass",
    subtitle = "Data (black), linear fit, and hyperbolic fits"
  ) +
  theme_classic() +
  theme(legend.position = "bottom")

# Choose higher-contrast colors (blue + orange) from your palette
col_blue   <- unname(fill_cols)[1]
col_orange <- unname(fill_cols)[4]

# Ryan/Spitters hyperbola ------------------------------------------------
if (!inherits(hyper_mod_ryan, "error")) {
  p_yield_totalweed <- p_yield_totalweed +
    stat_function(
      aes(color = "Hyperbolic (Ryan/Spitters)"),
      fun = function(x) {
        pars <- coef(hyper_mod_ryan)
        pars["Y0"] / (1 + pars["iw"] * x)
      },
      linewidth = 1,
      linetype = "dotdash"
...
# 3-parameter hyperbola + asymptote -------------------------------------
if (!inherits(hyper_mod_3p, "error")) {

  pars3  <- coef(hyper_mod_3p)
  y_asym <- as.numeric(pars3["Y0"] - pars3["Lmax"])

  # Add curve + dotted asymptote line (both in blue to tie to 3-parameter model)
  p_yield_totalweed <- p_yield_totalweed +
    stat_function(
      aes(color = "Hyperbolic (3-parameter)"),
      fun = function(x) {
        pars <- coef(hyper_mod_3p)
  ...
  # Place text clearly below the dotted line and away from points
x_lab <- max(dat_yield$B, na.rm = TRUE) * 0.42
y_rng <- diff(range(dat_yield$Y, na.rm = TRUE))
y_lab <- y_asym - 0.07 * y_rng   # more separation from the line

# plotmath label (no unicode)
asym_lab <- paste0("Asymptote == ", round(y_asym, 0), "~kg~ha^{-1}")
...
p_yield_totalweed <- p_yield_totalweed +
  ...
  # y-limits: include observed data and asymptote (with padding)
  y_min_data <- min(dat_yield$Y, na.rm = TRUE)
  y_max_data <- max(dat_yield$Y, na.rm = TRUE)

  y_min_plot <- min(y_min_data, y_asym)
  y_max_plot <- y_max_data
  pad <- 0.05 * (y_max_plot - y_min_plot)

  p_yield_totalweed <- p_yield_totalweed +
    coord_cartesian(ylim = c(y_min_plot - pad, y_max_plot + pad))
}
...
p_yield_totalweed <- p_yield_totalweed +
  scale_color_manual(
    name = NULL,
    breaks = c("Hyperbolic (Ryan/Spitters)", "Hyperbolic (3-parameter)"),
    values = c(
      "Hyperbolic (Ryan/Spitters)" = col_orange,
      "Hyperbolic (3-parameter)"   = col_blue
    )
  )

p_yield_totalweed

save_fig_rel(p_yield_totalweed, "yield_totalweed_fits", width = 7, height = 5)
...
# Quick range checks (useful later for plot limits / asymptote label placement)
summary(reg_biomass$B)
summary(reg_biomass$Y)
range(reg_biomass$B, na.rm = TRUE)
range(reg_biomass$Y, na.rm = TRUE)

```
#### Mixed model selection (site-year effects)
```{r}
### Mixed model selection (site-year effects): soybean biomass ----------
## Additive vs interaction (common slope vs site-year-specific slopes)

options(contrasts = c("contr.sum", "contr.poly"))

# 1) Candidate mixed models (fit with REML for estimation) --------------
biomass_add <- lmer(
  Y ~ B + site_year + (1 | site_year:block),
  data = reg_biomass,
  REML = TRUE
...
# 2) LRT + ML AICs (anova refits with REML = FALSE internally) ----------
lrt_biomass <- anova(biomass_add, biomass_int)  # ML comparison

AIC_add_biomass <- lrt_biomass$AIC[1]
AIC_int_biomass <- lrt_biomass$AIC[2]
deltaAIC_biomass_int_add <- AIC_int_biomass - AIC_add_biomass  # <0 favors interaction

p_int_biomass <- lrt_biomass$`Pr(>Chisq)`[2]

# 3) Decision rule (same structure as your weed-biomass rule) -----------
p_strong    <- 0.01
p_none      <- 0.20
dAIC_strong <- 4

interaction_class_biomass <- dplyr::case_when(
  p_int_biomass < p_strong | (-deltaAIC_biomass_int_add) >= dAIC_strong ~ "interaction",
  p_int_biomass > p_none   & abs(deltaAIC_biomass_int_add) < 2          ~ "additive",
  TRUE                                                                  ~ "gray_zone"
)

chosen_model_name_biomass <- dplyr::case_when(
  interaction_class_biomass == "interaction" ~ "Interaction: B * site_year",
  TRUE                                      ~ "Additive: B + site_year"
)

biomass_mod_best <- if (chosen_model_name_biomass == "Interaction: B * site_year") {
  biomass_int
} else {
  biomass_add
}

# 4) Output table for global regression summary -------------------------
biomass_mixedmodel_selection_out <- tibble(
  response_label    = "Soybean biomass (kg ha^-1)",
  predictor         = "Total weed biomass (kg ha^-1)",
  model_additive    = "B + site_year + (1|site_year:block)",
  model_interaction = "B * site_year + (1|site_year:block)",
  AIC_additive      = AIC_add_biomass,
  AIC_interaction   = AIC_int_biomass,
  deltaAIC_int_add  = deltaAIC_biomass_int_add,
  LRT_p_int         = p_int_biomass,
  selected_model    = chosen_model_name_biomass,
  interaction_class = interaction_class_biomass
)

# Save + print
save_tab_rel(biomass_mixedmodel_selection_out, "biomass_totalweed_mixedmodel_selection")

biomass_mixedmodel_selection_out |>
  mutate(
    across(c(AIC_additive, AIC_interaction, deltaAIC_int_add), ~ round(.x, 2)),
    LRT_p_int = round(LRT_p_int, 3)
  ) |>
  kable(
    caption = "Soybean biomass vs total weed biomass: mixed-model selection (additive vs site-year interaction)."
  ) |>
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"))
...
cat(
  ...
  sprintf("\n  LRT p (interaction vs additive) = %.3f; ΔAIC (int - add) = %.2f [%s]\n",
          p_int_biomass, deltaAIC_biomass_int_add, interaction_class_biomass)
)

```
#### Slope summary via emtrends
```{r}
## Slope summary (emtrends): soybean biomass vs total weed biomass -------

has_interaction_biomass <- chosen_model_name_biomass == "Interaction: B * site_year"

if (!has_interaction_biomass) {

  # Pooled slope across site-years
  emm_slope <- emtrends(biomass_mod_best, ~ 1, var = "B")
  tmp <- summary(emm_slope, infer = c(TRUE, TRUE)) |> as_tibble()
  ...
  # Site-year-specific slopes
  emm_slope <- emtrends(biomass_mod_best, ~ site_year, var = "B")
  tmp <- summary(emm_slope, infer = c(TRUE, TRUE)) |> as_tibble()
...
save_tab_rel(slope_tbl_biomass, "biomass_totalweed_slopes_emtrends")

slope_tbl_biomass |>
  mutate(across(where(is.numeric), ~ round(.x, 3))) |>
  kable(
    caption = "Slopes of soybean biomass vs total weed biomass (emtrends from selected mixed model)"
  ) |>
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"))
...
#### Linear vs rectangular hyperbolic response
```{r}
## 4) Linear vs hyperbolic: soybean biomass response (Ryan/Spitters; optional 3-parameter)
#### Linear vs rectangular hyperbolic response --------------------------
...
# Hyperbolic (Ryan/Spitters biomass form): Y = Y0 / (1 + iw * B)
start_vals_ryan_biomass <- list(
  Y0 = as.numeric(quantile(dat_biomass$Y, 0.95, na.rm = TRUE)),
  iw = as.numeric(1 / max(dat_biomass$B[dat_biomass$B > 0], na.rm = TRUE))
)

hyper_mod_ryan_biomass <- tryCatch(
  minpack.lm::nlsLM(
    Y ~ Y0 / (1 + iw * B),
    data    = dat_biomass,
    start   = start_vals_ryan_biomass,
    lower   = c(Y0 = 0, iw = 0),
    control = minpack.lm::nls.lm.control(maxiter = 500)
  ),
  error = function(e) e
...
# Optional: 3-parameter absolute-yield hyperbola: Y = Y0 - (Lmax*B)/(K+B)
start_vals_3p_biomass <- list(
  Y0   = as.numeric(quantile(dat_biomass$Y, 0.95, na.rm = TRUE)),
  Lmax = as.numeric(max(dat_biomass$Y, na.rm = TRUE) - min(dat_biomass$Y, na.rm = TRUE)),
  K    = as.numeric(median(dat_biomass$B[dat_biomass$B > 0], na.rm = TRUE))
)
if (!is.finite(start_vals_3p_biomass$K)) start_vals_3p_biomass$K <- 1

hyper_mod_3p_biomass <- tryCatch(
  minpack.lm::nlsLM(
    Y ~ Y0 - (Lmax * B) / (K + B),
    data    = dat_biomass,
    start   = start_vals_3p_biomass,
    lower   = c(Y0 = 0, Lmax = 0, K = 0),
    control = minpack.lm::nls.lm.control(maxiter = 500)
  ),
  error = function(e) e
...
mod_comp_biomass <- tibble(
  model = "Linear",
  df    = df.residual(lin_mod_biomass),
  AIC   = AIC(lin_mod_biomass),
  R2    = broom::glance(lin_mod_biomass)$r.squared
)

# Optional: parameter table (useful for global regression summary)
param_tbl_biomass <- tibble()

if (!inherits(hyper_mod_ryan_biomass, "error")) {
  mod_comp_biomass <- bind_rows(
    mod_comp_biomass,
    tibble(
      model = "Hyperbolic (Ryan/Spitters: Y0/(1+iwB))",
      df    = df.residual(hyper_mod_ryan_biomass),
      AIC   = AIC(hyper_mod_ryan_biomass),
      R2    = r2_from_fit(dat_biomass$Y, fitted(hyper_mod_ryan_biomass))
    )
  )

  param_tbl_biomass <- bind_rows(
    param_tbl_biomass,
    tibble(
      model    = "Ryan/Spitters",
      term     = names(coef(hyper_mod_ryan_biomass)),
      estimate = as.numeric(coef(hyper_mod_ryan_biomass))
    )
  )
} else {
  cat("\nRyan/Spitters hyperbolic fit failed (biomass):\n", hyper_mod_ryan_biomass$message, "\n")
}

if (!inherits(hyper_mod_3p_biomass, "error")) {
  mod_comp_biomass <- bind_rows(
    mod_comp_biomass,
    tibble(
      model = "Hyperbolic (3-parameter: Y0 - Lmax*B/(K+B))",
      df    = df.residual(hyper_mod_3p_biomass),
      AIC   = AIC(hyper_mod_3p_biomass),
      R2    = r2_from_fit(dat_biomass$Y, fitted(hyper_mod_3p_biomass))
    )
  )
  ...
  param_tbl_biomass <- bind_rows(
    ...
    tibble(
      model    = "3-parameter",
      term     = names(coef(hyper_mod_3p_biomass)),
      estimate = as.numeric(coef(hyper_mod_3p_biomass))
    )
  )
} else {
  cat("\n3-parameter hyperbolic fit failed (biomass):\n", hyper_mod_3p_biomass$message, "\n")
}

save_tab_rel(mod_comp_biomass, "biomass_totalweed_linear_vs_hyperbolic")
if (nrow(param_tbl_biomass) > 0) save_tab_rel(param_tbl_biomass, "biomass_totalweed_hyperbolic_parameters")

mod_comp_biomass |>
  mutate(across(where(is.numeric), ~ round(.x, 3))) |>
  kable(
    caption = "Marginal model comparison (no random effects): linear vs hyperbolic soybean biomass response to total weed biomass"
  ) |>
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"))
...
##### Table
```{r}
# 6) OPTIONAL: Combine current biomass ~ total weed biomass outputs into one workbook
# (robust: reads the CSVs you already saved)

biomass_totalweed_tables <- list(
  MixedModel_Selection = readr::read_csv(
    file.path(tab_dir_rel, "weed_soy-biomass_totalweed_mixedmodel_selection.csv"),
    show_col_types = FALSE
  ),
  Slopes_emtrends = readr::read_csv(
    file.path(tab_dir_rel, "weed_soy-biomass_totalweed_slopes_emtrends.csv"),
    show_col_types = FALSE
  ),
  Linear_vs_Hyperbolic = readr::read_csv(
    file.path(tab_dir_rel, "weed_soy-biomass_totalweed_linear_vs_hyperbolic.csv"),
    show_col_types = FALSE
  )
)

writexl::write_xlsx(
  biomass_totalweed_tables,
  path = file.path(tab_dir_rel, "weed_soy-biomass_totalweed_all-tables.xlsx")
)
...
p_biomass_totalweed <- ggplot(dat_biomass, aes(x = B, y = Y)) +
  geom_point(color = "black") +
  # Linear fit (quiet benchmark)
  stat_function(
...
# Ryan/Spitters hyperbola ------------------------------------------------
if (!inherits(hyper_mod_ryan_biomass, "error")) {
  p_biomass_totalweed <- p_biomass_totalweed +
    stat_function(
      aes(color = "Hyperbolic (Ryan/Spitters)"),
      fun = function(x) {
        pars <- coef(hyper_mod_ryan_biomass)
        pars["Y0"] / (1 + pars["iw"] * x)
      },
      linewidth = 1,
      linetype = "dotdash"
...
if (!inherits(hyper_mod_3p_biomass, "error")) {
  ...
  pars3  <- coef(hyper_mod_3p_biomass)
  y_asym <- as.numeric(pars3["Y0"] - pars3["Lmax"])
  ...
  p_biomass_totalweed <- p_biomass_totalweed +
    stat_function(
      aes(color = "Hyperbolic (3-parameter)"),
      fun = function(x) {
        pars <- coef(hyper_mod_3p_biomass)
        pars["Y0"] - (pars["Lmax"] * x) / (pars["K"] + x)
      },
      linewidth = 1,
      linetype = "twodash"
  ...
  # Place label below the dotted line and away from points
  x_lab <- max(dat_biomass$B, na.rm = TRUE) * 0.42
  y_rng <- diff(range(dat_biomass$Y, na.rm = TRUE))
  y_lab <- y_asym - 0.07 * y_rng
  ...
  # y-limits: include observed data and asymptote (with padding)
  y_min_data <- min(dat_biomass$Y, na.rm = TRUE)
  y_max_data <- max(dat_biomass$Y, na.rm = TRUE)
...
save_fig_rel(p_biomass_totalweed, "biomass_totalweed_fits", width = 7, height = 5)
